...
export function EventsDashboard({ token, onSignout }: EventsDashboardProps) {
  ...
  const [registrationsEvent, setRegistrationsEvent] = useState<EventItem | null>(null)
  ...
  function openEdit(event: EventItem) {
    ...
    setForm({ title: event.title, description: event.description ?? '', location: event.location ?? '', startDate: toInputDate(event.startDate), endDate: toInputDate(event.endDate), imageUrl: event.imageUrl ?? '', backgroundColor1: event.backgroundColor1 ?? '#083344', backgroundColor2: event.backgroundColor2 ?? '#06b6d4' })
    setShowForm(true)
  }
  ...
  if (selectedEvent) return <EventFieldsPage event={selectedEvent} token={token} onBack={() => setSelectedEvent(null)} onSignout={onSignout} onCheckIn={() => { setSelectedEvent(null); setShowCheckIn(true) }} />
  if (registrationsEvent) return <RegistrationsPage event={registrationsEvent} token={token} onBack={() => setRegistrationsEvent(null)} onSignout={onSignout} onCheckIn={() => { setRegistrationsEvent(null); setShowCheckIn(true) }} />
  if (showCheckIn) return <CheckInPage token={token} events={events} onBack={() => setShowCheckIn(false)} onSignout={onSignout} />

  return (
    <main className="flex min-h-screen bg-[#eef2ff] text-slate-700">
      <Sidebar onSignout={onSignout} onCheckIn={() => setShowCheckIn(true)} />
